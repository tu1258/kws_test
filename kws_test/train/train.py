...
def train(flags):
  ...
  training_steps_list = []
  steps_per_epoch = int(audio_processor.set_size('training') / flags.batch_size)
  for i in range(len(training_epochs_list)):
    training_steps_list.append(training_epochs_list[i] * steps_per_epoch)
  training_steps_max = np.sum(training_steps_list)
  lr_init = learning_rates_list[0]
  exp_rate = -np.log(learning_rates_list[-1] / lr_init)/training_steps_max
  logging.info('training_steps_max=%d', training_steps_max)

  # Training loop.
  if(flags.debug):
    end_step = 11
  else:
    end_step = training_steps_max + 1
  ...
  # for training_step in range(start_step, training_steps_max + 1):
    ...
    if (training_step % steps_per_epoch) == 0 or is_last_step:
  ...
  logging.info('last weights Final test accuracy = %.2f%% (N=%d)',
